...
const createAmbulanceProviderDB = async (
  email,
  phone_number,
...
const getAmbulanceProviderByIdDB = async (id) => {
  return ambulanceProviders.findByPk(id);
};

const updateAmbulanceProviderDB = async (id, updateData) => {
  const updatedRowsCount = await ambulanceProviders.update(updateData, {
    where: { id },
  });
  ...
  const updatedProvider = await ambulanceProviders.findByPk(id);
  return updatedProvider;
};

const deleteAmbulanceProviderDB = async (id) => {
  try {
    const deletedRowsCount = await ambulanceProviders.destroy({
      where: { id },
    });
